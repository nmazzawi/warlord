...
- **Found by review, after the first build:** a march to a named place now ENDS on that place (Rome's own point sits on a cell this coarse grid calls sea, so the warband stopped 23 units short and the city could never be entered); arrival picks the NEAREST place, not the first in the list, so Korinthos, Kawa and Abdju are no longer mistaken for the bigger city beside them; a double click whose first half lands on a panel's MARCH button takes the march back instead of walking; lifting one finger out of a pinch no longer flings the map; the first map of a page load sits below the status bar like every later one; a wheel zoom interrupts the LOCATE flight instead of losing to it; a country tells its three stories once, not once per city; and every realm's forge sells at least one thing your own camp does not, so the walk is never for nothing.

## TOTAL WAR, TOTAL FREEDOM (designer, 2026-08-29)

Any settlement in any land-reachable realm can be attacked at any time. There are no "not yet" locks on war — the lock is consequences. Foreign realms defend with overwhelming, realm-tiered garrisons built from existing unit templates plus ONE culture-flavoured elite per realm for now (Roman Legionaries: shielded formation infantry; Rus Druzhina: heavy axemen; Persian Immortals: armoured spears that refill their ranks; etc.) — each realm's true bespoke roster and mechanics still arrive with its own milestone later.

- **Strength tiers:** neighbouring fringe villages are hard but possible for a strong warband; great-empire towns are brutal; capitals are effectively suicide until late game. Settlement panels state defender counts honestly so the choice is informed, never forbidden.
- **Consequences:** attacking anything in a realm spikes that realm's own infamy meter — its gates shut to you, its hunter patrols pursue you inside its borders. Realms remember.
- **If you somehow win:** the normal sack / occupy / leave choice applies, and occupation works generically (tribute, garrison, shops) until that realm's culture milestone deepens it.
- **Sea-locked realms** stay unreachable only because no ship exists yet.
- Delete every "war comes in a later age" message; replace it with defender intel.

## Milestone 4.10 — The world goes hot (built 2026-08-29)

**Working assumptions made by the build — overrule by feel:**
- **Every place in a reachable realm is a place you can march on.** The nine land realms grew from 36 visitable cities to all 80 of their settlements — thrones, great cities, towns and the villages on their fringe. Tapping any of them gives you its garrison, its realm's own elite, and an ASSAULT button. Nothing anywhere says no.
- **The ladder is the lock.** A garrison is the place's rank multiplied by the realm's own strength: a Kushite village keeps 18 men, a Rus village 20, a Roman town 44, Kiev 48, Roma 58 — the last at more than three and a half times the stats of a homeland militiaman. Kush is the softest country on the board and Rome the hardest, and the panel prints the exact number before you decide.
- **One elite each, for now.** Nine realms, three fighting styles drawn from what those men actually did: Rome's Legionaries and the Ghilman, Strong-Arms and Kandake's Shieldmen lock shields — **half of every blow is turned while the shield is up, and they are only open in the moment they swing**. Rus Druzhinniki and China's Horse-Cutters fight with both hands on a long blade: slow, telegraphed, enormous. Greece's Epilektoi, Persia's Immortals and India's Padarakshas hold a spear that out-reaches every sword — and **the ranks close over their dead**, so killing one puts another in the gap, a fixed number of times. Each realm's own roster arrives with its own milestone; these are drawn from one silhouette apiece, painted in that country's colours.
- **Realms remember.** Touching anything of theirs writes it against that realm's own meter, never your homeland's: a village is 16, which is exactly the point at which a country starts hunting. Their gates shut across the whole realm the moment you draw steel in it, and their riders come after you inside their borders — their own men, not a generic patrol.
- **If you win, it is yours.** Sack, occupy or leave, exactly as at home. An occupied foreign city pays 8–45 gold a day by rank, its workshops open to you at local prices, and its people take your coin to fight — a conquered city is not a place you are a foreigner in any more.
- **No battle fields more than 58 men**, because a phone has to draw all of them. Above that the rank and file are trimmed and the elites are never touched. Measured at 59 fps with all 58 of Roma awake at once.

## MILESTONE 5 — THE FOURTEEN THRONES (designer, 2026-08-30)

This absorbs the old Milestone 7 (culture starts). Governance and coalition milestones shift later.

**A. ATLAS POLISH (finish the chart).** Remove the scattered sea creatures and symbols from the oceans entirely. The compass rose sits over open ocean only (the South Atlantic works) and never over land. Remove the outer border frame, the "THE KNOWN WORLD" cartouche and the scale bar — a clean edge-to-edge map. Then a cartography quality pass: relief-shaded mountain ranges, textured forest and desert fills, more detailed coastlines, subtle paper grain, refined italic ocean labels. It should read as a real atlas plate.

**B. CHOOSE YOUR CIVILIZATION (new-game flow, Feudalism 2 spirit).** A full-screen selection of 15 starts — the 14 civilizations plus **The Borderland Outlaw**, the current start, kept as the hard neutral option. Each start shows: a named hero with a code-drawn portrait figure in cultural dress, a 3–5 sentence backstory (grounded, evocative, each hinting at that culture's fighting identity), the home realm highlighted on a mini-chart, a culture-appropriate starting weapon, 3 starting troops from that culture's roster, and a one-line playstyle note. Choosing spawns your home camp inside that realm; your home realm then behaves exactly as the Borderland does now — gates open, normal prices, barracks unlocked, and its infamy meter is your "homeland" meter, which raiding at home still spikes. Deep per-culture rulesets (Horde momentum, Doctrine, and the rest) remain a later milestone: for now the differentiation is roster + kit + home region + hero stat lean + backstory. Existing saves load as the Borderland Outlaw.

**C. TROOP ROSTERS (every culture, on the five-stat framework).** 3–4 recruitable unit types per civilization: a cheap line unit, an elite, and a specialist with a visible signature ability; distinct recruit costs AND daily wages. Patterns to build from — Rome: Legionary (shield line), Velite (javelin skirmisher), Centurion (nearby troops fight harder); Japan: Ashigaru, Samurai, Ninja; Mongols: Steppe Rider, Lancer, Khan's Guard; Vikings: Raider, Huscarl, Berserker (never routs, frenzies at low HP); Kush: massed archer line plus a shieldbearer. A barracks stocks its own settlement's culture roster, so occupation produces mixed armies as designed. A pre-battle formation pick, one tap: **LINE** (troops form ahead) / **WEDGE** (troops flank the sides) / **GUARD** (troops ring the hero).

**D. LIVING SETTLEMENTS (replace the card-grid screens).** Every settlement screen becomes an illustrated scene, its buildings composed procedurally by tier and culture, with distinct architecture sets — pagodas, columns, domes, longhouses, yurts, adobe. Tap a building to open its panel: **MARKETPLACE** (buy and sell weapons, armour and loot, with a notice board carrying simple quests — deliver a parcel to settlement X for gold, or kill a named hunter party), **FORGE** (upgrades and refinement bonuses), **BARRACKS**, **STABLES**, **INN** (rumors and quests), and **HARBOR** on coastal settlements (flavour now, ships later). Loot economy: elite enemies and captains sometimes drop their gear, and anything you own can be sold at any open marketplace.

**E. STARS.** Every settlement shows a 1–5 star protection rating on its map plate and in its panel, alongside the honest defender intel — so you can see at a glance what you can plausibly take.

### Build order (proposed by the build, 2026-08-30)
1. **M5.1 — the chart finished**: A + E. Self-contained, and the whole map is judged on it.
...
- **Found while testing M5.2:** a Japanese start had no Japan to stand in. Settlements only existed for realms a road could reach, so the four across the water had none. Every realm's settlements are nodes now — 116 of them — and whether you can WALK to one is the pathfinder's business, not the list's. The four sea realms were given everything the other nine had: a forge and stables that sell, an inn that sells their own rumors, a barracks shut in their own words, and the elite their garrisons field (Samurai, Huscarls, Jaguar Warriors, Sinchikuna).
- **Three starts are confined**, and the card says so: Japan, the Aztecs and the Inca have no road out, so until there are ships the whole run is fought inside one country of nine settlements. The Norse are not confined — Scandinavia joins Rus by land. A country you cannot reach shows no MARCH button and says why.

## Milestone 5.3 — Armies of the world (built 2026-08-30)

**Working assumptions made by the build — overrule by feel:**
- **Fifty-eight units, nine behaviours.** Every signature its author wrote maps onto one of nine things a player can SEE: *javelin* throws once on the way in; *frenzy* hits harder the more it bleeds; *backstab* opens a man who is busy with somebody else; *inspire* makes everyone near it fight harder while it stands (and draws the ring it does it in); *duel* goes for the strongest man on the field rather than the nearest; *trample* catches whoever is standing beside its target; *shieldwall* turns most of a blow but is open in the moment its own is going out; *volley* keeps its distance; *lance* rides. A player learns nine rules, not fifty-eight.
- **Every man is paid what he is worth.** A samurai is not a levy in the ledger any more than on the field: wages are summed per unit, so a warband of elites bleeds gold and a warband of levies does not.
- **One tap before every fight.** LINE puts them in front of you, WEDGE spreads them to the flanks, GUARD rings you. It is asked every time because the answer is different every time — a lane wants a line, a plaza wants a wedge, a field of archers wants them round you. Nothing moves until it is answered, the battle's own title waits its turn, and the choice is remembered between fights.

## Milestone 5.4 — Living settlements (built 2026-08-30)

**Working assumptions made by the build — overrule by feel:**
- **A settlement is a street, not a list.** Sky, a far skyline of this country's own roofs, ground, and a row of buildings standing on it — each one a tap target from its roof down to the line under its name. The card grid is gone.
- **Seven sets for fourteen countries**, because neighbours really did build alike: classical columns and pediments (Rome, Greece); tiered roofs with upturned eaves (Japan, China); domes and horseshoe arches (the Caliphate, Persia, India); steep timber halls with crossed gables (the Norse, Rus); round felt tents (the steppe); stepped mudbrick (Egypt, Kush, the Aztecs, the Inca); and plain timber for the Borderland. The silhouette does the work — the far skyline is the same shapes again, small and hazy, so a capital feels like a capital.
...
## Milestone 5.5 — depth and the crown (built 2026-08-30)

**Working assumptions made by the build — overrule by feel:**
- **A garrison is made of something, not just sized.** How well a place is held decides its mix: a border hamlet is four farmers with spears and nothing else; a town is militia, bowmen, captains and a handful of the country's own; a throne is the reverse — Roma fields 14 militia to 20 Legionaries, with a champion in front. The panel prints the composition exactly as it will spawn, champion included, and he counts against the 58 bodies a phone has to draw.
- **Two levers on the whole warband, not on one man.** Any forge that will serve you arms everybody — three tiers, priced by the head, so a big warband is a big bill — and the pay rate is HALF, FULL or DOUBLE. Half is cheap and men walk off in the night; double buys a warband that never deserts and swings a little harder. The ledger says which you are paying.
- **A crown is taken, not granted.** Hold a country's throne AND every one of its great cities and the rest sends fealty — no walking to each village to finish a war that is already over. The realm becomes yours: its score against you clears, its hunts stop for good, its gates open at its own prices, its tribute is yours, and they call you by its own word for the man who holds it — Imperator, Shogun, Khan, Pharaoh, Qore, Sapa Inca. A crown on its name on the chart, and the title in your status bar where the bounty used to be.
- **Kings are noticed.** Every other country that has not bent the knee moves up one rung the day you take a throne. This is the first thread of the world-responds system: nothing you do is local any more.

## Milestone 5.5 — towns that look like towns (built 2026-08-30)
...
- **A boat is always there.** Every harbour, every day, whatever your name is worth and whoever you have crossed — there is a hull on the tide and a man willing to take your silver. Passage is never refused, never out of season, never gated behind a reputation or a quest. The only thing that ever stops you sailing is not having the fare.
...
## Milestone 5.7 — every place gets its own ground (built 2026-09-01)

**Working assumptions made by the build — overrule by feel:**
- **Real geography stacks a capital's suburbs on top of it, and this chart cannot afford that.** A day's march is about 140 world units; Tlacopan was drawn 2 units from Tenochtitlan, Ostia 5 from Roma, Marathon 3 from Athenai. Eight Aztec settlements stood inside 25 units of their throne — one marker's worth of ground holding a third of a country, where only the capital could be named and a tap could not tell you which of them you had hit.
- **So the atlas is eased apart once, at load.** Thrones do not move; cities give way a little, towns more, villages most. Nothing is pushed into the sea, off its own landmass, or (where it can be helped) out of its own country, and a move is only ever taken if it leaves a place FURTHER from its nearest neighbour than it was — the pass may fail to help, but it may never make anything worse.
- **How far apart a country can hold its towns is a fact about the country.** Rome has a continent and can spare sixty units a settlement; Japan is nine settlements on forty-six grid cells and the valley of Mexico has nine cities and three hamlets to fit on a hand's breadth of land. Each realm is spread to a little more than it already had, not to a number chosen in advance.
- **Markers grow slower than the ground.** Past the close band they still get bigger on screen, but they take up less of the world as they do — otherwise the crowding is frozen and zooming in reveals nothing new. Zooming in has to mean something.
- **Close in, a place is always drawn.** Far out it may step aside entirely; from arm's length it always keeps its marker, even where there is no room to write its name. Tapping something that was never drawn is worse than a crowded chart.

## Milestone 6 — Ships (built 2026-09-01)

**Working assumptions made by the build — overrule by feel:**
- **The sea had to be repaired before it could be sailed.** The terrain grid asks whether the CENTRE of each 18-unit cell is land, and Gibraltar is drawn about five units wide — so the strait closed and the Mediterranean became a lake. Flooded as drawn, the world's water is eleven bodies: an ocean, a sealed Mediterranean of 351 cells holding every Roman and Greek port, a sealed Red Sea, and eight puddles. Twenty-seven harbours could reach each other and nowhere else on Earth. Three cells are opened by hand — Gibraltar and two at Bab el-Mandeb — and the water becomes one sea.
- **A harbour is where a hull can lie, not where you can see the sea.** The settlement scene's coast test looks 46 units out, which on this chart is 200 km — at that range Damascus, Jerusalem and Persepolis are ports. A harbour is deep water within one cell, under a day's walk to the quay: 62 of them. Three countries the atlas gives no coast worth the name — Rus, Persia, Kush — get one authored river port each (Novgorod, Susa, Amara West), because a country with no way to sea is a country whose people must walk into somebody else's to take ship. The Borderland and the steppe stay landlocked, and that is the honest answer: the outlaw start marches to the sea.
- **A ship sails through the night.** About four and a half times a marching column, measured against the land router. Ostia to Sakai is 52 days and 728 gold; Athens to Reykjavik 17 days; Massilia to the Aztec coast 22. Long, and not a season of nothing.
